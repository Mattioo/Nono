...
void SC08A::SetInputRange(unsigned int min, unsigned int max) {
  if (min < max) {
...
void SC08A::Set(const std::vector<unsigned char>& channelsToSet, unsigned int position, unsigned char velocity) { 
    // SCALE POSITION SIGNAL

    unsigned int position_before_scale = position;
