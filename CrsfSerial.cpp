...
void CrsfSerial::packetChannelsPacked(const crsf_header_t *p)
{
    ...
    for (unsigned int i=0; i<CRSF_NUM_CHANNELS; ++i)
        _channels[i] = scaleSignal(map(_channels[i], CRSF_CHANNEL_VALUE_1000, CRSF_CHANNEL_VALUE_2000, CRSF_CHANNEL_VALUE_MIN, CRSF_CHANNEL_VALUE_MAX));
...
int CrsfSerial::scaleSignal(int crsfSignal){
  int transformed = min(max(crsfSignal, CRSF_CHANNEL_VALUE_MIN), CRSF_CHANNEL_VALUE_MAX);
  
  return abs(transformed - CRSF_CHANNEL_VALUE_MID) < CRSF_SIGNAL_OFSET
